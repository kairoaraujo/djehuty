...
class CollectionService:
    ...
    def list_collections(self, *, limit=10, offset=0, order="published_date",
                         order_direction="desc", categories=None, doi=None,
                         handle=None, groups=None, institution=None,
                         modified_since=None, published_since=None,
                         resource_doi=None, search_for=None,
                         is_latest=True, account_uuid=None,
                         **kwargs) -> list[dict]:
        """Return a list of formatted collection summaries."""
        records = self.db.collections(
            limit=limit, offset=offset, order=order,
            order_direction=order_direction, categories=categories,
            doi=doi, handle=handle, group=groups, institution=institution,
            modified_since=modified_since, published_since=published_since,
            resource_doi=resource_doi, search_for=search_for,
            is_latest=is_latest, account_uuid=account_uuid,
        )
        return [
            formatter.format_collection_record({**r, "base_url": config.base_url})
            for r in records
        ]

    def search_collections(self, *, limit=10, offset=0, order="published_date",
                           order_direction="desc", account_uuid=None,
                           **kwargs) -> tuple[list[dict], int | None]:
        """Search collections, returning results and total count."""
        records = self.db.collections(
            limit=limit, offset=offset, order=order,
            order_direction=order_direction, is_latest=True,
            account_uuid=account_uuid, **kwargs,
        )
        formatted = [
            formatter.format_collection_record({**r, "base_url": config.base_url})
            for r in records
