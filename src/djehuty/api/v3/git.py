...
@router.get("/datasets/{dataset_id}.git/files", summary="List files in default branch")
def git_files(
    dataset_id: str,
    ...
    if branch_name:
        try:
            ...
            raise InvalidInputError(
                f"Failed to retrieve Git files for '{branch_name}' "
                f"in '{repository.path}': {error}",
                "GitReadFailed",
            )
    return JSONResponse(content=files)


@router.get("/datasets/{dataset_id}.git/branches", summary="List branches + default")
def git_branches(
    dataset_id: str,
    account=Depends(require_auth),
    db=Depends(get_db),
...
def git_set_default_branch(
    ...
    if not isinstance(branch_name, str):
        raise InvalidInputError(
            "Field 'branch' is required.", "BadBranch"
        )
    if branch_name not in repository.branches.local:
        raise InvalidInputError(
            f"Branch '{branch_name}' does not exist.", "UnknownBranch"
        )

    if not git_service.set_default_branch(repository, branch_name):
        raise InvalidInputError(
            "Failed to set default branch.", "SetDefaultFailed"
        )
...
def _resolve_repo_for_auth(
    db, git_uuid: str, token: str | None, write: bool
) -> str:
...
def _git_cgi(
    repository_path: str, environ: dict, body: bytes | None = None,
    ...
    env["PATH_INFO"] = "/" + os.path.basename(repository_path) + env.get(
        "PATH_INFO", ""
    )
...
@router.get("/datasets/{git_uuid}.git/languages", summary="Languages in default branch")
...
@router.get("/datasets/{git_uuid}.git/contributors", summary="Commit-author counts")
...
def git_zip(git_uuid: str, db=Depends(get_db)):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="{git_uuid}.zip"'
            )
        },
    )
